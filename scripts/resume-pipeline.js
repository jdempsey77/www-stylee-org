...
class ResumePipeline {
  ...
  // Parse the Word document content (simplified for text-based docx)
  parseWordDocument() {
    console.log('📄 Parsing Word document...');
    
    try {
      // For now, we'll read the text content we created
      // In a real implementation, you'd use a library like 'mammoth' or 'docx-parser'
      const content = fs.readFileSync(this.docPath, 'utf8');
      
      const resumeData = this.extractResumeData(content);
  ...
  // Extract structured data from the resume content
  extractResumeData(content) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (i === 0) {
      ...
      } else if (currentSection === 'summary' && line && !line.startsWith('PROFESSIONAL')) {
        resumeData.summary += (resumeData.summary ? ' ' : '') + line;
      } else if (currentSection === 'experience') {
        // Parse experience entries
        if (!line.startsWith('•') && !line.includes('|') && line && !line.includes('PROFESSIONAL')) {
          // This is a job title
        ...
        if (line.includes('Security Technologies:')) {
          // Parse security skills
        } else if (line.includes('Programming Languages:')) {
          // Parse programming skills
        } else if (line.includes('Cloud Platforms:')) {
          // Parse cloud skills
        } else if (line.includes('Security Frameworks:')) {
...
<body>
  ...
  async run() {
    try {
      console.log('🔄 Starting resume pipeline...\n');
      
      // Step 1: Parse Word document
      const resumeData = this.parseWordDocument();
      ...
      console.error('\n❌ Resume pipeline failed:', error.message);
      process.exit(1);
    }
  }
}
